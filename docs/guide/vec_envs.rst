...
.. automodule:: deeprl.common.vec_env
...
For consistency across DeepRL and because of its special requirements and features,
DeepRL VecEnv API is not the same as Gym API.
DeepRL VecEnv API is actually close to Gym 0.21 API but differs to Gym 0.26+ API:
...
- at the end of an episode, ``infos[env_idx]["TimeLimit.truncated"] = truncated and not terminated``
  ...
  The conversion from DeepRL to Gym API is

  .. code-block:: python

    # done is True at the end of an episode
    # dones[env_idx] = terminated[env_idx] or truncated[env_idx]
    # In DeepRL, truncated and terminated are mutually exclusive
...
.. code-block:: python
	...
	# DeepRL uses VecEnv for training, where `env.unwrapped.x = new_value` cannot be used to set an attribute
...
In this example ``env.mu`` cannot be accessed/changed directly because it is wrapped in a ``VecEnv`` and because it could be wrapped with other wrappers.
...
.. code-block:: python
	...
	from deeprl.common.vec_env.base_vec_env import VecEnv, VecEnvStepReturn, VecEnvWrapper
