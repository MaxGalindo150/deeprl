...
.. code-block:: python
  ...
  # DeepRL provides SimpleMultiObsEnv as an example environment with Dict observations
...
and multiprocessing for you. To install the Atari environments, run the command ``pip install gymnasium[atari,accept-rom-license]`` to install the Atari environments and ROMs, or install DeepRL with ``pip install deeprl[extra]`` to install this and other optional dependencies.

.. .. image:: ../_static/img/colab-badge.svg
..    :target: link to the notebook
..
...
.. code-block:: python
    ...
    from deeprl.common.vec_env import VecNormalize
    from deeprl.common.env_util import make_vec_env
    from deeprl import PPO
...
.. code-block:: python
  ...
  from deeprl import HerReplayBuffer, SAC, DDPG, TD3
  from deeprl.common.noise import NormalActionNoise
...
.. pretrain models with a linear learning rate schedule?
...
However, DeepRL provides a ``save_replay_buffer()`` and ``load_replay_buffer()`` method to save it separately.
...
.. code-block:: python
  ...
  from deeprl import SAC
  from deeprl.common.evaluation import evaluate_policy
  from deeprl.sac.policies import MlpPolicy
...
DeepRL and ProcgenEnv
------------------

Some environments like `Procgen <https://github.com/openai/procgen>`_ already produce a vectorized
environment. In order to use it with DeepRL, you must wrap it in a ``VecMonitor`` wrapper which will also allow
...
.. code-block:: python
  ...
  from deeprl import PPO
  from deeprl.common.vec_env import VecExtractDictObs, VecMonitor
...
DeepRL with EnvPool or Isaac Gym
-----------------------------
...
To use DeepRL with those tools, you must wrap the env with tool's specific ``VecEnvWrapper`` that will pre-process the data for DeepRL,
...
.. code-block:: python
  ...
  import gymnasium as gym
  from deeprl.common.vec_env import VecVideoRecorder, DummyVecEnv
