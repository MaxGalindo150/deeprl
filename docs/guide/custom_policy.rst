...
DeepRL provides policy networks for images (CnnPolicies),
...
DeepRL networks are separated into two mains parts (see figure below):
...
.. image:: ../_static/img/policy_arch.png

.. note::

    - **Features Extractor**: The features extractor is shread by default between actor/critic networks (if applicable).
      It is used to preprocess the observations before fully-connected layers.

    - **Net Arch**: The network architecture can be partially shared between actor/critic networks (if applicable). 

DeepRL policies are usually composed of several networks (actor/critic networks + target networks when applicable) together with the associated optimizers.

Each of these networks have a featrue extractor followed by a fully-connected network.

.. note::

  When we refer to "policy" in DeepRL, this is usually an abuse of language compared to RL terminology.
  In DeepRL, "policy" refers to the class that handles all the networks useful for training,
  so not only the network used to predict actions (the "learned controller").

Default Network Architecture
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The default network architecture used by DeepRL depends on the algorithm and the observation space.
...
.. code-block:: python
  ...
  from deeprl import PPO
  from deeprl.common.torch_layers import BaseFeaturesExtractor
...
DeepRL supports handling of multiple inputs by using ``Dict`` Gym space. This can be done using
...
.. code-block:: python
  ...
  from deeprl import PPO
  from deeprl.common.policies import ActorCriticPolicy
