...
class QLearningPolicy(BasePolicy):
    def __init__(
        ...
        optimizer_class: Type[th.optim.Optimizer] = th.optim.Adam,
        optimizer_kwargs: Optional[Dict[str, Any]] = None,
        lr_schedule: Schedule = lambda _: 1e-3,
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False) -> th.Tensor:
        state = observation.item()  # Assuming observation is a single integer state
        if deterministic:
    ...
    def update_q_table(self, state: int, action: int, reward: float, next_state: int, done: bool, alpha: float, gamma: float):
        best_next_action = np.argmax(self.q_table[next_state])
        td_target = reward + gamma * self.q_table[next_state][best_next_action] * (1 - done)
        td_error = td_target - self.q_table[state][action]
        self.q_table[state][action] += alpha * td_error

    def learn(self, state: int, action: int, reward: float, next_state: int, done: bool, alpha: float = 0.1, gamma: float = 0.99):
        self.update_q_table(state, action, reward, next_state, done, alpha, gamma)