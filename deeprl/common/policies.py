...
class BaseModel(nn.Module):
    ...
    In the case of policies, the prediction is an action. In the case of critics, it is the estimated value of the observation.
    
    :param observation_space: The observation space of the environment
    :param action_space: The action space of the environment
    :param features_extractor_class: Feature extractor to use
    :param features_extractor_kwargs: Keyboard arguments
    ...
    def __init__(
        ...
        if not normalize_images and issubclass(features_extractor_class, (NatureCNN, CombinedExtractor)):
            self.features_extractor_kwargs.update(dict(normalize_images=True))
        ...
        def _update_features_extractor(
            ...
            """"
            Update the network keyword arguments and crate a new features extractor object if needed.
            If a ``features_extractor_`` object is passed, then it will be shared.
            
            :param net_kwargs: the base network keyword, without the ones related to features extractor
            ...
            if features_extractor is None:
                # The featrues extractor is not shared, create a new one
                features_extractor = self.make_features_extractor()
            net_kwargs.update(dict(features_extractor=features_extractor, features_dim=features_extractor.features_dim))
            return net_kwargs
        
        def make_features_extractor(self) -> BaseFeaturesExtractor:
            """"Helps method to create a feature extractor."""
            return self.features_extractor_class(self.observation_space, **self.features_extractor_kwargs)
        
        def extract_features(self, obs: PyTorchObs, features_extractor: BaseFeaturesExtractor) -> th.Tensor:
            """
            Prepocess the observation if needed and extract features.
            
            :param obs: The input observation
            :param features_extractor: The feature extractor to use.
        ...
        def _get_constructor_parameters(self) -> Dict[str, Any]:
            ...
            :return: The dictionary to pass to the as kwargs constructor when reconstructing the model.
            """
            return dict(
        ...
        def device(self) -> th.device:
            ...
            If it has no parameters, the 'cpu' device is used as fallback.
        ...
        @classmethod
        def load(cls: Type[SelfBaseModel], path: str, device: Union[th.device, str]):
            """
            ...
            # Note(antonin): we cannot use `wieghts_only=True` here because we need to allow
            # gymnasium imports for the policy to be loaded succesfully.
