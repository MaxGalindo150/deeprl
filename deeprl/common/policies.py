...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        # Tuple obs are not supported by DRL, so we can safely do that check
        if isinstance(observation, tuple) and len(observation) == 2 and isinstance(observation[1], dict):
            raise ValueError(
                "You have passed a tuple to the predict() function instead of a Numpy array or a Dict. "
                "You are probably mixing Gym API with DRL VecEnv API: `obs, info = env.reset()` (Gym) "
                "vs `obs = vec_env.reset()` (DRL VecEnv). "
...
class ActorCriticPolicy(BasePolicy):
    ...
    :param full_std: Whether to use (n_featurers x n_actions) parameters
        for std instead of only (n_features,) when using gSDE.
    ...
    :param features_extractor_class: Features extractor to use.
    :param featrues_extractor_kwargs: Keyword arguments to pass to the features extractor.
    :param share_features_extractor: Whether to share the feature extractor
    ...
    def __init__(
        ...
        if isinstance(net_arch, list) and len(net_arch) > 0 and isinstance(net_arch[0], dict):
            warnings.warn(
                "DeepRLearn does not support shared layers in the mlp_extractor, "
        ...
        if self.share_features_extractor:
            ...
            self.pi_features_extractor = self.vf_features_extractor = self.features_extractor
        else:
            self.pi_features_extractor = self.make_features_extractor()
            self.vf_features_extractor = self.make_features_extractor()
        
        #The features dim is always defined by the actor
        ...
        # Keyword argunments for gSDE distribution
        if use_sde:
            dist_kwargs = {
             "full_std": full_std,
