...
from deeprl.common.buffers import DictReplayBuffer, ReplayBuffer
...
SelfOffPolicyAgent = TypeVar("SelfOffPolicyAgent", bound="OffPolicyAgent")


class OffPolicyAgent(BaseAgent):
    """
    The base for Off-Policy agents (ex: SAC/TD3)
    ...
    :param train_freq: Update the model every ``train_freq`` steps. Alternatively pass a tuple of
        like ``(5, "step")`` or a ``(2, "episode")``.
    :param gradient_steps: How many gradient steps to take after each rollout (see ``train_freq``)
    ...
    :param use_sde_at_warmup: Whether to use gSDE instead of uniform samplin
    ...
    def __init__(
        ...
        sde_sample_freq: int = -1, # Sample a new noise matrix every n steps when using gSDE
    ...
    def _convert_train_freq(self) -> None:
        ...
        if not isinstance(self.train_freq, tuple):
            ...
            try:
                ...
                raise ValueError(f"Invalid `train_freq` value: {train_freq}") from e
            if not isinstance(train_freq[0], int):
                raise ValueError(f"Invalid `train_freq` value: {train_freq}")
