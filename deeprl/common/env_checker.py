...
from deeprl.common.preprocessing import check_for_nested_spaces, is_image_space_channels_first
from deeprl.common.vec_env import DummyVecEnv, VecCheckNan
...
def _check_image_input(observation_space: spaces.Box, key: str = "") -> None:
    """
    Check that the input will be compatible with DeepRL
    ...
    if observation_space.shape[non_channel_idx] < 36 or observation_space.shape[1] < 36:
        warnings.warn(
            ...
            "cf. https://deeprl/guide/custom_policy.html"
        )


def _check_unsupported_spaces(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    """Emit warnings when the observation space or action space used is not supported by DeepRL."""
    ...
    if isinstance(observation_space, spaces.Dict):
        ...
        if nested_dict:
            warnings.warn(
                "Nested observation spaces are not supported by DeepRL "
    ...
    if isinstance(observation_space, spaces.MultiDiscrete) and len(observation_space.nvec.shape) > 1:
        warnings.warn(
            ...
            "which is currently not supported by DeepRL. "
    ...
    if isinstance(observation_space, spaces.Tuple):
        warnings.warn(
            ...
            "this is currently not supported by DeepRL. "
    ...
    if isinstance(observation_space, spaces.Sequence):
        warnings.warn(
            "Sequence observation space is not supported by DeepRL. "
    ...
    if not _is_numpy_array_space(action_space):
        warnings.warn(
            ...
            "This type of action space is currently not supported by DeepRL. You should try to flatten the "
...
def check_env(env: gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    ...
    It also optionally check that the environment is compatible with DeepRL.

    :param env: The Gym environment that will be checked
    :param warn: Whether to output additional warnings
        mainly related to the interaction with DeepRL
    ...
    # A warning means that the environment may run but not work properly with DeepRL algorithms
    if warn:
        ...
        if isinstance(action_space, spaces.Box) and (
            ...
            warnings.warn(
                ...
                "cf. https://deeprl/guide/rl_tips.html"
            )
